...
app = Flask(__name__)
CORS(app)  # Fix CORS cho GitHub Pages
...
def generate_key():
    ...
    # Generate unique 4-digit key (0000-9999)
    conn = sqlite3.connect("serverkey.db")
    c = conn.cursor()
    max_retries = 20
    key = None
    for attempt in range(max_retries):
        ...
        c.execute("SELECT 1 FROM keys WHERE key=?", (candidate,))
        if not c.fetchone():
            key = candidate
            break
    if not key:
        conn.close()
        return jsonify({'error': 'Could not generate unique 4-digit key after retries'}), 500
...
def serve_admin(path):
    if path != '' and path != 'admin.html':
        return "Not found", 404
    try:
        return send_from_directory('.', 'admin.html')
    except FileNotFoundError:
        return "<a href='/admin.html'>Go to Admin Panel</a>", 302

@app.route("/admin")
def admin_redirect():
    return redirect(url_for('serve_admin', path='admin.html'))
